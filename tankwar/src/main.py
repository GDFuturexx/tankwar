...
def main():
    ...
    # 创建地图（也就是显示砖块和铁块），其实就是实例化wall.py中的Map方法
    bgMap = wall.Map()
    # 创建食物/道具 但不显示
    prop = food.Food()
    # 创建我方坦克，把坦克加入到相应组当中去
    ...
    # 自定义事件
    # 创建敌方坦克延迟200毫秒，创建一个然后技能冷却为false，当200毫秒触发这个延迟事件则设置为true
    # 比如pygame.constants.USEREVENT定义了一个用户事件，这事件是延时事件
    # constants是一个常量，这里得到的是一个id，用户的id
    DELAYEVENT = pygame.constants.USEREVENT
    # 这个就像setInterval间隔函数，就是每200毫秒触发这个延迟事件，循环的200毫秒就为true，就等于设置了一个冷却时间
    # pygame.time.set_timer在队列中循环创建一个事件
    ...
    # 这都是我们需要定义的初始值
    # 
    # 这里定义delay用于下面计算判断真假是程序运行用的，初始值可修改
    delay = 100
    # 坦克1的移动的距离
    moving = 0
    # 坦克1移动的方向
    ...
    while True:
        ...
        # 画砖块
        # 也可以用pygame.sprite。Group中的update和draw方法实现这个渲染
